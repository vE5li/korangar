...
//     App: Application,
// {
//     fn get_text_dimensions(&self, text: &str, font_size: App::FontSize,
// available_width: f32) -> App::Size; }
...
//     );
//
//     fn render_text(&self, text: &str, position: App::Position, clip:
// App::Clip, color: App::Color, font_size: App::FontSize) -> f32;
//
//     fn render_checkbox(&self, position: App::Position, size: App::Size, clip:
// App::Clip, color: App::Color, checked: bool);
//
//     fn render_expand_arrow(&self, position: App::Position, size: App::Size,
// clip: App::Clip, color: App::Color, expanded: bool); }
...
//         let height = self.height().expect("element cannot have flexible
// height");
//
...
//     }
//
//     pub fn set_focused_element(&mut self, element: Option<ElementCell<App>>,
// window_index: usize) {         self.focused_element =
// element.as_ref().map(Rc::downgrade);         self.focused_window =
// Some(window_index);     }
//
...
//     }
//
//     pub fn update_focused_element(&mut self, element:
// Option<ElementCell<App>>, window_index: usize) {         if let Some(element)
// = element {             self.focused_element = Some(Rc::downgrade(&element));
//             self.focused_window = Some(window_index);
//         }
//     }
//
...
//     }
//
//     pub fn did_hovered_element_change(&self, hovered_element:
// &Option<ElementCell<App>>) -> bool {         self.previous_hovered_element
//             .as_ref()
//             .zip(hovered_element.as_ref())
//             .map(|(previous, current)| !Weak::ptr_eq(previous,
// &Rc::downgrade(current)))
// .unwrap_or(self.previous_hovered_element.is_some() ||
// hovered_element.is_some())     }
//
...
//             .unwrap_or(self.previous_focused_element.is_some() ||
...
//     }
//
//     pub fn update(&mut self, hovered_element: &Option<ElementCell<App>>,
// window_index: Option<usize>) -> Option<ElementCell<App>> {         self.
// previous_hovered_element = hovered_element.as_ref().map(Rc::downgrade);
...
//
//         self.focused_element.clone().and_then(|weak_element|
