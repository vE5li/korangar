mod vertex_shader {
    vulkano_shaders::shader! {
        ty: "vertex",
        path: "shaders/water_light_vertex_shader.glsl"
    }
}
...
mod fragment_shader {
    vulkano_shaders::shader! {
        ty: "fragment",
        path: "shaders/water_light_fragment_shader.glsl"
    }
}
...
impl WaterLightRenderer {
    ...
    pub fn render(&self, builder: &mut CommandBuilder, camera: &dyn Camera, water_buffer: ImageBuffer, depth_buffer: ImageBuffer, vertex_buffer: ScreenVertexBuffer, water_level: f32) {

        let layout = self.pipeline.layout().clone();
        let descriptor_layout = layout.descriptor_set_layouts().get(0).unwrap().clone();

        let set = PersistentDescriptorSet::new(descriptor_layout, [
            WriteDescriptorSet::image_view(0, water_buffer),
            WriteDescriptorSet::image_view(1, depth_buffer),
        ]).unwrap(); 
