...
fn main() {
    ...
    time_phase!("create window", {
        ...
        let mut image_data = image_buffer.as_bytes().to_vec();
        ...
        let icon = sdl2::surface::Surface::from_data(
            ...
            image_buffer.width(),
            image_buffer.height(),
            image_buffer.width() * PixelFormatEnum::RGBA8888.byte_size_per_pixel() as u32,
    ...
    time_phase!("create adapter and surface", {
        ...
        window
            .set_title(&format!("{CLIENT_NAME} ({})", str::to_uppercase(&backend)))
            .unwrap();
    ...
    time_phase!("configure surface", {
        ...
        let mut surface = Surface::new(adapter, device.clone(), surface, width, height);
    ...
    'running: loop {
        for event in event_pump.poll_iter() {
            match event {
                ...
                Event::Window {
                    ...
                    surface.update_window_size([width as u32, height as u32]);
                    continue;
                }
                Event::Window {
                ...
                Event::MouseMotion { x, y, .. } => input_system.update_mouse_position(x, y),
                Event::MouseButtonDown { mouse_btn, .. } => input_system.update_mouse_buttons(mouse_btn, true),
                Event::MouseButtonUp { mouse_btn, .. } => input_system.update_mouse_buttons(mouse_btn, false),
                Event::MouseWheel { precise_y, .. } => input_system.update_mouse_wheel(precise_y),
                Event::KeyDown { keycode, .. } => {
                    if let Some(keycode) = keycode {
                        input_system.update_keyboard(keycode, true);
                    }
                }
                Event::KeyUp { keycode, .. } => {
                    if let Some(keycode) = keycode {
                        input_system.update_keyboard(keycode, false);
                    }
                }
        ...
        for event in network_events {
            match event {
                ...
                NetworkEvent::MapServerDisconnected { reason } => {
                    ...
                    let character_selection_window =
                        CharacterSelectionWindow::new(saved_characters.new_remote(), move_request.new_remote(), saved_slot_count);
                ...
                NetworkEvent::CharacterSelected { login_data, map_name } => {
                    ...
                    map = map_loader.get(map_name, &mut model_loader, &mut texture_loader).unwrap();
                ...
                NetworkEvent::AddQuestEffect(quest_effect) => particle_holder.add_quest_icon(&mut texture_loader, &map, quest_effect),
                ...
                NetworkEvent::IventoryItemAdded { item } => {
                    player_inventory.add_item(&mut texture_loader, &script_loader, item);

                    // TODO: Update the selling items. If you pick
                    // up an
                    // item that you already
                    // have the sell window should allow you to sell
                    // the
                    // new amount of items.
                }
                NetworkEvent::InventoryItemRemoved {
                    ...
                    player_inventory.remove_item(index, amount);
                }
                ...
                NetworkEvent::AddSkillUnit(entity_id, unit_id, position) => match unit_id {
                    UnitId::Firewall => {
                        ...
                        let effect = effect_loader.get("firewall.str", &mut texture_loader).unwrap();
                        let frame_timer = effect.new_frame_timer();
                    ...
                    UnitId::Pneuma => {
                        let position = Vector2::new(position.x as usize, position.y as usize);
                        let position = map.get_world_position(position);
                        let effect = effect_loader.get("pneuma1.str", &mut texture_loader).unwrap();
                        let frame_timer = effect.new_frame_timer();
                ...
                NetworkEvent::OpenShop { items } => {
                    shop_items.mutate(|shop_items| {
                        *shop_items = items
                            .into_iter()
                            .map(|item| script_loader.load_market_item_metadata(&mut texture_loader, item))
                            .collect()
                    });
                    ...
                    interface.open_window(
                        &application,
                        &mut focus_state,
                        &BuyWindow::new(shop_items.new_remote(), cart.clone()),
                    );
                ...
                NetworkEvent::SellItemList { items } => {
                    ...
                    interface.open_window(
                        &application,
                        &mut focus_state,
                        &SellWindow::new(sell_items.new_remote(), cart.clone()),
                    );
            ...
            match event {
                ...
                UserEvent::OpenGraphicsSettingsWindow => interface.open_window(
                    ...
                    &GraphicsSettingsWindow::new(
                        surface.present_mode_info(),
                        shadow_detail.clone_state(),
                        framerate_limit.clone_state(),
                    ),
                ),
                ...
                #[cfg(feature = "debug")]
                UserEvent::OpenThemeViewerWindow => interface.open_window(&application, &mut focus_state, application.theme_window()),
                #[cfg(feature = "debug")]
                UserEvent::OpenProfilerWindow => interface.open_window(&application, &mut focus_state, &ProfilerWindow::new()),
                #[cfg(feature = "debug")]
                UserEvent::OpenPacketWindow => interface.open_window(
                    &application,
                    &mut focus_state,
                    &PacketWindow::new(packet_callback.remote(), PlainTrackedState::default()),
                ),
                #[cfg(feature = "debug")]
        ...
        if surface.is_invalid() {
            ...
            screen_targets = (0..max_frame_count).map(|_| deferred_renderer.create_render_target()).collect();

            interface_target = interface_renderer.create_render_target();

            picker_targets = (0..max_frame_count).map(|_| picker_renderer.create_render_target()).collect();
        }
        ...
        let mut picker_render_command_encoder = device.create_command_encoder(&CommandEncoderDescriptor {
            label: Some("Picker render"),
        });
        let mut picker_render_pass = picker_target.start_render_pass(&mut picker_render_command_encoder);

        let mut picker_compute_command_encoder = device.create_command_encoder(&CommandEncoderDescriptor {
            label: Some("Picker compute"),
        });
        let mut picker_compute_pass = picker_target.start_compute_pass(&mut picker_compute_command_encoder);

        let mut interface_command_encoder = device.create_command_encoder(&CommandEncoderDescriptor { label: Some("Interface") });
        let mut interface_render_pass = interface_target.start(&mut interface_command_encoder, clear_interface);

        let mut directional_shadow_command_encoder = device.create_command_encoder(&CommandEncoderDescriptor { label: Some("Shadow") });
        let mut directional_shadow_render_pass = directional_shadow_target.start(&mut directional_shadow_command_encoder);

        let mut point_shadow_command_encoder = device.create_command_encoder(&CommandEncoderDescriptor {
            label: Some("Point Light"),
        });

        let mut geometry_command_encoder = device.create_command_encoder(&CommandEncoderDescriptor { label: Some("Geometry") });
        let mut geometry_render_pass = deferred_target.start_geometry_pass(&mut geometry_command_encoder);

        let mut screen_command_encoder = device.create_command_encoder(&CommandEncoderDescriptor { label: Some("Screen") });
