...
// }
//
// impl<const KIND: InternalThemeKind> PrototypeElement<InterfaceSettings> for
// ThemeSelector<KIND> {     fn to_element(&self, display: String) ->
// ElementCell<InterfaceSettings> {         let state =
// PlainTrackedState::new(self.0.clone());
//
...
//             .filter_map(|path| {
//                 let name =
// path.path().file_name()?.to_str()?.strip_suffix(".ron")?.to_owned();
...
//     fn default() -> Self {
//         let fonts = Vec::from_iter(DEFAULT_FONTS.iter().map(|font|
// font.to_string()));         let main_theme =
// "client/themes/main.ron".to_string();         let menu_theme =
// "client/themes/menu.ron".to_string();         let game_theme =
// "client/themes/game.ron".to_string();         let scaling =
// Scaling::new(1.0);
//
//         Self {
//             fonts,
...
//             #[cfg(feature = "debug")]
//             print_debug!("failed to load interface settings from {}",
...
//         #[cfg(feature = "debug")]
//         print_debug!("loading interface settings from {}",
// Self::FILE_NAME.magenta());
//
...
//     pub fn save(&self) {
//         #[cfg(feature = "debug")]
//         print_debug!("saving interface settings to {}",
// Self::FILE_NAME.magenta());
//
//         let data = ron::ser::to_string_pretty(self,
// PrettyConfig::new()).unwrap();         std::fs::write(Self::FILE_NAME,
...
//             // scaling: MutableRange::new(scaling, Scaling::new(0.5),
// Scaling::new(2.5)),             themes,
//         }
//     }
//
//     // TODO: Remove
...
//     pub fn set_theme_file(&mut self, theme_file: String, kind:
// InternalThemeKind) {         match kind {
...
//         match kind {
//             InternalThemeKind::Menu =>
// self.themes.menu.save(self.menu_theme.get_file()),
// InternalThemeKind::Main => self.themes.main.save(self.main_theme.get_file()),
//             InternalThemeKind::Game =>
// self.themes.game.save(self.game_theme.get_file()),         }
//     }
//
//     #[cfg_attr(feature = "debug", korangar_debug::profile)]
...
//         match kind {
//             InternalThemeKind::Menu =>
// self.themes.menu.reload::<DefaultMenu>(self.menu_theme.get_file()),
//             InternalThemeKind::Main =>
// self.themes.main.reload::<DefaultMain>(self.main_theme.get_file()),
//             InternalThemeKind::Game =>
// self.themes.game.reload(self.game_theme.get_file()),         }
//     }
// }

// impl Appli for InterfaceSettings {
