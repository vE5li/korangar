...
impl DialogContainer {
    fn to_element(
        ...
        match dialog_element {
            DialogElement::Text(text) => Text::new(font_loader)
    ...
    pub fn new(font_loader: Rc<RefCell<FontLoader>>, dialog_elements: PlainRemote<Vec<DialogElement>>, npc_id: EntityId) -> Self {
        let elements = dialog_elements
            .get()
            .iter()
            .map(|element| Self::to_element(font_loader.clone(), element, npc_id))
            .collect();
...
impl Element<InterfaceSettings> for DialogContainer {
    ...
    fn update(&mut self) -> Option<ChangeEvent> {
        if self.dialog_elements.consume_changed() {
            *self = Self::new(self.font_loader.clone(), self.dialog_elements.clone(), self.npc_id);
