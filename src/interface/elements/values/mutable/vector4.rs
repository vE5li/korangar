...
use interface::windows::Vector4Window;
use interface::types::*;
...
impl<T: Zero + NumOps + NumCast + Copy + PartialOrd + Display + 'static> Element for MutableVector4Value<T> {
    ...
    fn resolve(&mut self, placement_resolver: &mut PlacementResolver, _interface_settings: &InterfaceSettings, theme: &Theme) {
        ...
        //self.cached_values = format!("{:.1}, {:.1}, {:.1}, {:.1}", self.cached_inner.x, self.cached_inner.y, self.cached_inner.z, self.cached_inner.w);
    }
    ...
    fn left_click(&mut self, _force_update: &mut bool) -> Option<ClickAction> {
        Some(ClickAction::OpenWindow(Box::new(Vector4Window::new(self.name.clone(), self.inner_pointer, self.minimum_value, self.maximum_value, self.change_event.clone()))))
    }
