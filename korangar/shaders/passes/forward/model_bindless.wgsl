...
const SHADOW_OFFSET_NORMAL: f32 = -0.2;
const SHADOW_OFFSET_LIGHT: f32 = -0.2;
const CONSTANT_DEPTH_BIAS: f32 = 0.0001;

// Based on: https://www.ludicon.com/castano/blog/articles/shadow-mapping-summary-part-1/
fn get_shadow_offsets(normal: vec3<f32>, light_direction: vec3<f32>) -> vec2<f32> {
    ...
    return vec2<f32>(sin_alpha, min(2.0, tan_alpha));
}
...
fn fragment(input: VertexOutput) -> vec4<f32> {
    ...
    if (PASS_MODE == 0 && alpha_channel == 0.0) {
        // Opaque pass we exit early on invisible pixels.
        discard;
    } else  if (PASS_MODE == 1 && (alpha_channel == 0.0 || alpha_channel <= (1.0 - OPAQUE_EPSILON))) {
        // Semi opaque pass can early exit if invisible or transparent pixel was found.
        discard;
    } else if (PASS_MODE == 2 && (alpha_channel == 0.0 || alpha_channel > (1.0 - OPAQUE_EPSILON))) {
        ...
        let biased_world_position = input.world_position + vec4<f32>(displacement_N + displacement_L, 0.0);

        let shadow_position = directional_light_partitions[partition_index].view_projection * biased_world_position;
        ...
        shadow_coords = vec3<f32>(
            ...
            max(shadow_coords.z, 0.0) + CONSTANT_DEPTH_BIAS
        );
