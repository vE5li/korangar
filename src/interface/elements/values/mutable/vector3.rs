...
use crate::interface::{Element, Vector3Window, *};
...
impl<T: Zero + NumOps + NumCast + Copy + PartialOrd + Display + 'static> Element for MutableVector3Value<T> {
    ...
    fn update(&mut self) -> Option<ChangeEvent> {
        ...
        if self.cached_inner != current_value {
            ...
            self.cached_values = format!(
                "{:.1}, {:.1}, {:.1}",
                self.cached_inner.x, self.cached_inner.y, self.cached_inner.z
            );
    ...
    fn left_click(&mut self, _force_update: &mut bool) -> Option<ClickAction> {
        ...
        Some(ClickAction::OpenWindow(Box::new(Vector3Window::new(
            self.name.clone(),
            self.inner_pointer,
            self.minimum_value,
            self.maximum_value,
