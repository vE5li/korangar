...
impl Display for FromFileError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FromFileError::NoDefaultTrack => f.write_str("Could not determine the default audio track"),
            FromFileError::UnknownSampleRate => f.write_str("Could not detect the sample rate of the audio"),
            FromFileError::UnsupportedChannelConfiguration => f.write_str("Only mono and stereo audio is supported"),
