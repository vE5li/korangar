...
fn calculate_sample(position: vec4<f32>, fragment_position: vec2<f32>, sample_index: i32) -> vec3<f32> {
    ...
    var light_percent: f32 = dot(normalize(-matrices.direction.xyz), normal);
    ...
    let light_position: vec4<f32> = matrices.light * pixel_position_world_space;
    var light_coords: vec3<f32> = light_position.xyz / light_position.w;

    let uv: vec2<f32> = clip_to_screen_space(light_coords.xy);
    let shadow_map_depth: f32 = textureSample(shadow_map, shadow_sampler, uv);
    ...
    return light_percent * matrices.color.rgb * diffuse * visibility;
}
