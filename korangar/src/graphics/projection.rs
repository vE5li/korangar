...
/// to 0.0 (far).
pub fn orthographic_reverse_lh(left: f32, right: f32, bottom: f32, top: f32) -> Matrix4<f32> {
    let width = 1.0 / (right - left);
    let height = 1.0 / (top - bottom);
    let depth = 1.0 / (FAR_PLANE - NEAR_PLANE);

    Matrix4::from_cols(
        Vector4::new(width + width, 0.0, 0.0, 0.0),
        Vector4::new(0.0, height + height, 0.0, 0.0),
        Vector4::new(0.0, 0.0, -depth, 0.0),
        Vector4::new(-(left + right) * width, -(top + bottom) * height, depth * FAR_PLANE, 1.0),
    )
}
