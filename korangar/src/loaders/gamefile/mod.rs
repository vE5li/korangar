...
const LUA_FILE_NAME: &str = "lua_files.7z";
...
impl GameFileLoader {
    ...
    pub fn remove_patched_lua_files(&self) {
        if Path::new(LUA_FILE_NAME).exists() {
            std::fs::remove_file(LUA_FILE_NAME).unwrap();
        }
    }
    ...
    pub fn load_patched_lua_files(&self) {
        ...
        let lua_archive = Self::load_archive_from_path(LUA_FILE_NAME);
    ...
    fn patch_lua_files(&self) {
        ...
        let path = Path::new(LUA_FILE_NAME);
